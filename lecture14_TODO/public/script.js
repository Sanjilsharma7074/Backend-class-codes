...
clearBtn.addEventListener("click", async () => {
  await axios.delete("http://localhost:4000/todo/clear-completed");

  // reload according to active filter
  if (currentFilter === "all") getAllTodos();
  else getFilterTodos(currentFilter);
});
...
todoContainer.addEventListener("click", async (e) => {
  ...
  if (btnClass == "delete") {
    const res = await axios.delete(
      `http://localhost:4000/todo/delete/${todoId}`
    );
  }
